...
**Waterfall: moderate-high with CV, ~75-85%.** Proportions and counts are recoverable (CV hit 26% and 73% against public truth). Individual bar depths are capped by resolution and by the subgroup color-mask question. The hybrid dependence is now measured, not asserted: across four hand-labeled harvested waterfalls (27 values) the VLM scored 0.22 within tolerance and missed 16 outright, while CV on the same figures read the extremes within 1 to 4 percent and hit the negative-threshold counts exactly wherever bars were separable. CV's ceiling is set by bar density (bars merge on the 54- and 185-bar panels), not by accuracy, and by the manual per-figure calibration each read requires.
